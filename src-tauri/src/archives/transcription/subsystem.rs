...
use crate::archives::transcription::constants::{SAMPLER_CHANNEL_SIZE, TRANSCRIBER_CHANNEL_SIZE};
// internal
...
use crate::{
    archives::{transcription::recorder::AudioRecorder, utils::data_channel::DataChannel},
    globals::Chunk,
};
...
pub struct ParakeetTranscriber {
    ...
    sampler_channel: Option<DataChannel<Chunk>>,
    transcriber_channel: Option<DataChannel<Chunk>>,
}
...
impl AudioTranscriber for ParakeetTranscriber {
    fn start_record_audio(&mut self) -> Result<(), TranscriptionError> {
        ...
        let sampler_channel: DataChannel<Chunk> = DataChannel::<Chunk>::new(SAMPLER_CHANNEL_SIZE);

        let transcriber_channel: DataChannel<Chunk> =
            DataChannel::<Chunk>::new(TRANSCRIBER_CHANNEL_SIZE);
        ...
        self.sampler_channel = Some(sampler_channel);
        self.transcriber_channel = Some(transcriber_channel);
        Ok(())
    }
